...
public class HeadSlot {
	...
	public HeadSlot(int x, int y, int ox, int oy, Inventory inv) {
		this.x = x;
		this.y = y;
	...
	public void render(Graphics g) {
		...
		Images.ITEMS.getSubImage(1, 3).draw(ox + (x * Constants.TILE_SIZE + (Inventory.SLOT_SPACING * x)), oy + (y * Constants.TILE_SIZE + (Inventory.SLOT_SPACING * y)));
		g.translate(ox, oy);
		if(hasItem()) {
			item.render(g);
		}
		...
		if(tooltip) {
			...
			if(hasItem()) {
				...
				g.setColor(Color.black);
				g.fillRect(x, y-25, Fonts.CLASS_TEXT.getWidth("Head"), 20);
				g.setColor(Color.white);
				g.setFont(Fonts.CLASS_TEXT);
				g.drawString("Head", x, y-23);
			}
		}
	}
	...
	public void onClick(int button, GameContainer gc) {
		...
		if(!hasItem() && inv.getInventoryManager().hasItem() && button == Input.MOUSE_LEFT_BUTTON && inv.getInventoryManager().getItem() instanceof Equipment) {
			if(((Equipment) inv.getInventoryManager().getItem()).getGearType() == Equipment.HEAD) {
